...
export default function ClientDetailPage() {
  ...
  const loadAll = async () => {
    ...
    try {
      ...
      // Load tasks for all orders
      if (ordersRes.data && ordersRes.data.length > 0) {
        const oids = ordersRes.data.map((o: Order) => o.id)
        // Fetch tasks one by one (PHP bridge doesn't support IN for multiple values well in all cases)
        const allTasks: Task[] = []
  ...
  const revealPassword = async (credId: string, encryptedPass: string | null) => {
    ...
    if (s.has(credId)) { s.delete(credId) }
      ...
      if (!decrypted[credId]) {
        ...
        setDecrypted(prev => ({ ...prev, [credId]: data.result }))
      }
    }
    setShowPass(s)
  }
  ...
  const approveClient = async () => {
    ...
    showToast(`Activated as ${data.client_id} — Welcome email sent!`, 'success')
    loadAll()
  }
          ...
          {clientOrders.length === 0 ? (
            ...
            return (
              <div key={order.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
                ...
                <Link href="/admin/clients/orders"
                  className="mt-3 flex items-center gap-1 text-xs text-blue-600 hover:text-blue-700 font-semibold">

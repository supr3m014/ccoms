...
if (isset($_GET['action']) && $_GET['action'] === 'run-migration') {
  ...
  $statements = [
    ...
    // ── CONTENT TABLES (base tables the admin editors write to) ──
    "CREATE TABLE IF NOT EXISTS pages (id VARCHAR(36) NOT NULL PRIMARY KEY DEFAULT (UUID()), title VARCHAR(500) NOT NULL, slug VARCHAR(500) NOT NULL, content LONGTEXT, excerpt TEXT, status ENUM('draft','published','scheduled','private') DEFAULT 'draft', visibility ENUM('public','private','password_protected') DEFAULT 'public', meta_title VARCHAR(500), meta_description TEXT, og_image VARCHAR(1000) DEFAULT NULL, author_id INT DEFAULT NULL, published_at TIMESTAMP NULL DEFAULT NULL, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, UNIQUE KEY uniq_pages_slug (slug)) ENGINE=InnoDB",
    "CREATE TABLE IF NOT EXISTS posts (id VARCHAR(36) NOT NULL PRIMARY KEY DEFAULT (UUID()), title VARCHAR(500) NOT NULL, slug VARCHAR(500) NOT NULL, content LONGTEXT, excerpt TEXT, status ENUM('draft','published','scheduled','private') DEFAULT 'draft', visibility ENUM('public','private','password_protected') DEFAULT 'public', comments_enabled TINYINT(1) DEFAULT 1, meta_title VARCHAR(500), meta_description TEXT, author_id INT DEFAULT NULL, published_at TIMESTAMP NULL DEFAULT NULL, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, UNIQUE KEY uniq_posts_slug (slug)) ENGINE=InnoDB",
    "CREATE TABLE IF NOT EXISTS blog_posts (id VARCHAR(36) NOT NULL PRIMARY KEY DEFAULT (UUID()), title VARCHAR(500) NOT NULL, slug VARCHAR(500) NOT NULL, excerpt TEXT, content LONGTEXT, featured_image VARCHAR(1000), status ENUM('draft','published','scheduled') DEFAULT 'draft', meta_title VARCHAR(500), meta_description TEXT, author VARCHAR(255) DEFAULT 'John Paul Carrasco', published_at TIMESTAMP NULL DEFAULT NULL, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, UNIQUE KEY uniq_blog_slug (slug)) ENGINE=InnoDB",
    "CREATE TABLE IF NOT EXISTS contact_submissions (id VARCHAR(36) NOT NULL PRIMARY KEY DEFAULT (UUID()), name VARCHAR(255) NOT NULL, email VARCHAR(255) NOT NULL, phone VARCHAR(50), company VARCHAR(255), message TEXT, archived TINYINT(1) DEFAULT 0, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP) ENGINE=InnoDB",
    "CREATE TABLE IF NOT EXISTS case_studies (id VARCHAR(36) NOT NULL PRIMARY KEY DEFAULT (UUID()), slug VARCHAR(255) NOT NULL, title VARCHAR(500) NOT NULL, subtitle VARCHAR(500), category VARCHAR(255), type ENUM('web-dev','seo') DEFAULT 'web-dev', images JSON, objective TEXT, highlights JSON, technical_summary JSON, quote_content JSON, seo_content JSON, external_proof_link VARCHAR(1000) DEFAULT NULL, live_url VARCHAR(1000) DEFAULT NULL, sort_order INT DEFAULT 0, status ENUM('draft','published') DEFAULT 'published', created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, UNIQUE KEY uniq_cs_slug (slug)) ENGINE=InnoDB",
    "CREATE TABLE IF NOT EXISTS team_members (id VARCHAR(36) NOT NULL PRIMARY KEY DEFAULT (UUID()), name VARCHAR(255) NOT NULL, role VARCHAR(255), department VARCHAR(255), photo_url VARCHAR(1000), sort_order INT DEFAULT 0, is_active TINYINT(1) DEFAULT 1, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP) ENGINE=InnoDB",
    // pages columns
